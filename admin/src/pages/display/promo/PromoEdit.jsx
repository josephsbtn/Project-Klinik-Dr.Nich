...
const PromoEdit = () => {
  ...
  useEffect(() => {
    const fetch =  async () =>{
      await axios.get( `${import.meta.env.VITE_BASE_URL_BACKEND}/api/promo/getPromoById/${id}`).then(
        response => {
          setData(response.data)
          setName(response.data.nama)
          setDeskripsi(response.data.detail)
          setSyarat(response.data.syarat)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `${import.meta.env.VITE_BASE_URL_BACKEND}/api/promo/updatepromo/`+id,
        fdata,
        {
          headers: {
      ...
      if (response.status === 200) {
        toast.success("Berhasil Mengaupdate Promo");
        setTimeout(() => {
      ...
      toast.error(
        error.response?.data?.message || "Gagal Mengaupdate Promo, coba lagi!"
      );
    }
  };
      ...
      <div className="flex flex-col gap-3">
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            <img
              src={imageDesktopRef?.current?.files[0] ?  imagePreview1  : data?.fotoDesktop}
              alt=""
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            <img
              src={imageMobileRef?.current?.files[0] ? imagePreview2 : data?.fotoMobile}
              alt=""
              className="h-[115px] w-[115px] rounded shadow-lg border"
            />
            <div>
